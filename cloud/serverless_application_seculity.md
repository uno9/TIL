...
#### SAS-10: 不適切な例外処理と詳細なエラーメッセージ
- サーバーレスアプリケーション
  - サーバーレスアプリケーションの開発を行うさいに、従来の開発と比較してデバッグ機能が制限されている
  - そのため、ファンクション内にテストコードを記述した開発を行う
  - しかし、テストコードやエラー文を表示する設定がある状態でユーザーへサービスを提供してしまうと、アプリケーションの内部ロジックが丸見えに
  - 弱点、脆弱性、欠陥、機密データの漏洩に繋がる
- 対策
  - サーバーレスアーキテクチャが提供するデバックツールを活用する
  - エラー文を設定する場合は、内部のロジックなどがわからないようなシンプルな文を設定すること

#### SAS-11: レガシー、または利用されていないサーバーレスファンクション、リソース、イベントトリガー
- サーバーレスファンクションを常に更新する
  - すでに廃止とされたサーバーレスアプリケーションコンポ―ネントを使用しないように剪定する
  - 更新されていないファンクションが脆弱性となり、攻撃をうける可能性がある
    - すでに使用していないサーバーレスファンクションが攻撃された場合、攻撃に気づかず影響範囲が拡大する可能背もある
- 剪定例：
  ...
  - 未使用のソフトウェア依存関係
- 対策
  - 古いサーバーレスアセット、クラウドリソース、設定、ファンクションを都度剪定する
    - クラウドセキュリティ態勢管理（CSPM）ソリューション、サーバーレスセキュリティプラットフォーム（SSP）を使用して、上記を検出する

#### SAS-12: CROSS-EXECUTION DATA PERSISTENCY
- コールドスタート回避の目的で、後続関数の呼び出しに実行環境を再利用することがある
  - 環境が再利用により、実行環境が異なるユーザーやセッションコンテキストに属する後続の呼び出し時に前回実行のセンシティブなデータが残され、露見する可能性がある
  - サーバーレスの環境を一時的なものかつステートレスなものをとして扱う必要がある
- 悪用例
  - 関数を実行時に、前回の実行データを意図的に破棄していない場合、前回データへアクセスされて情報がもれる可能性がある
- 対策
  - > ディスク、メモリ、環境変数に保存されたデータを含むローカルに保存されたデータが、同じランタイム環境で同じ関数を実行する間持続する可能性があることに注意してください
  - 機密情報が流出するのを防ぐため、暗号化されたクラウドシークレットセッションストレージ機能を使用する
  - 環境が再利用されるケースの場合、実行後のデータ破棄を行う必要がある
  - サーバーレスセキュリティプラットフォームを使用して、実行中のファンクションに対するセンシティブなデータへの不正アクセスと漏洩の監視、対策を行う必要がある

## サーバーレスアプリケーションの開発で気を付けること

1. クラウド面
  - クラウドベンダーが提供するベストクラティス設定を実施
  - 都度、設定を見直す
  - アプリケーションに適切かつ最小限の権利を付与する
2. アプリケーション面
   - 数珠繋ぎファンクション構成のため、どのファンクションが何に対して操作をするのか、しっかり設計を考えて開発する
   - 使用するライブラリの安全性を確認する
   - 重要な機密情報をべた書きしていないか、テストコードが残されていないか確認する
